  ...
  <v-container>
    ...
    >
      ...
      <template v-slot:item.actions="{ item }">
        <v-icon size="small" class="me-2" @click="viewAlarmDetails(item.id)">
          mdi-eye
        </v-icon>
      </template>
    </v-data-table>
...
const filteredAlarms = computed(() => {
  return alarms.value.filter((alarm) => {
    const statusMatch =
      !selectedStatus.value || alarm.status === selectedStatus.value;
    const dateMatch =
      !dateRange.value.length ||
      (new Date(alarm.date) >= new Date(dateRange.value[0]) &&
        new Date(alarm.date) <= new Date(dateRange.value[1]));
